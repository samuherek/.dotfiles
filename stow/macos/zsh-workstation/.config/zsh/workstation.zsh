VIM="nvim"

# Let's set colored prompt
PS1='%F{blue}%n@%m%f %F{cyan}%1~%f %# '

export EDITOR=$VIM
export GIT_EDITOR=$VIM

# Not sure where this comes from
PATH="/usr/local/bin:$PATH"
# this is rust related. They wanted me to do this
PATH="$HOME/.cargo/bin:$PATH"
# this is go related. They wanted me to do this
PATH="$(go env GOPATH)/bin:$PATH"
# This is personal bin scripts
PATH="$HOME/.bin:$PATH"
#Ruby shit
PATH="$HOME/.rbenv/bin:$PATH"
# this is required byte the python3 virtualenv
# I need to move this to the work related things instead of having this
# in the global zhsrc.
PATH="$PATH:$HOME/Library/Python/3.9/bin"
# This seems to be required for the Postgres.app. Otherwise Python3
# can not find the "pg_config" bin that is required for some packages.
PATH="/Applications/Postgres.app/Contents/Versions/latest/bin:$PATH"

#Path to local dev project
PATH="$PATH:$HOME/personal/ghi/target/release"
PATH="$PATH:$HOME/personal/dirpin/target/debug"

export PATH

# direnv hook so that it loads .envrc in directory
eval "$(direnv hook zsh)"

# copy absolute path to a file into clipboard
function pwdc() {
    local val="$1"
    if [ -z "$val" ]; then
        echo "No file or directory specified."
        return 0
    fi

    if [ -e "$val" ]; then
        local value="$(pwd)/$val"
        echo "copied $value"
        echo "$value" | pbcopy
    else
        echo "File or directory does not exist."
        return 0
    fi
}

# Quick nav into a dir in a monorepo like app
# Currently testing if this is useful
function in() {
    local dir=$(pwd)
    local selected=$(find "$dir" -type d -mindepth 1 -maxdepth 3 ! -path '*/.git/*' ! -path '*/node_modules/*' | fzf)

    if [[ -n $selected ]]; then
        cd "$selected"
    fi
}

# ZLE widget: quick directory navigation
quick-dir-widget() {
  local selected
  selected=$(quick_dir) || return
  [[ -z $selected ]] && return
  if [[ -d $selected ]]; then
    builtin cd -- "$selected"
    zle reset-prompt
  else
    zle -M "Not a directory: $selected"
  fi
}
zle -N quick-dir-widget
bindkey '^F' quick-dir-widget

# ZLE widget: pick a command from your zsh history with fzf, insert at cursor
h-widget() {
  local cmd
  cmd=$(fc -ln 1 | awk '!seen[$0]++' | fzf) || return
  LBUFFER+="$cmd"
}
zle -N h-widget
bindkey '^H' h-widget

export NVM_DIR="$HOME/.config/nvm"
[ -s "$NVM_DIR/nvm.sh" ] && . "$NVM_DIR/nvm.sh"
[ -s "$NVM_DIR/bash_completion" ] && . "$NVM_DIR/bash_completion"

#Ruby shit
eval "$(rbenv init -)"
